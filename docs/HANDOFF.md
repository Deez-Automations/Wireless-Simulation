...
### 2026-07-26 — System Model Slide Fix, and Contribution 2 Scoped (Power-Aware AP Association)

**What was done:**

1. **Fixed the System Model slide in `docs/CY315-Wireless-and-Mobile-Security.pptx`.** The single "System Model" slide (old slide 3) had two real errors: the AP-to-user text was backwards relative to the diagram (said AP1→User1/AP4→User2; diagram actually shows AP1→User2, AP4→User1), and the "Friendly Jammers" bullet incorrectly repeated the data-transmitter content instead of naming AP2/AP3. While fixing this, found that the **picture embedded in the pptx was stale** — it still showed the old 2-color convention (all four Eve-links drawn red, no blue at all), not matching the corrected `docs/AP1.png` on disk (2 green data links, 2 red Eve-intercept links, 2 blue jamming links). Fixed by replacing the embedded picture shape with the current `docs/AP1.png` via PowerPoint COM automation. Split the single slide into two — **"System Model: Setup"** (environment, threat model, corrected node roles) and a new **"System Model: Why It Works"** (signal legend, shared-spectrum interference, why the optimization favors Eve's degradation, secrecy-capacity result) — both using the corrected diagram. Verified visually by exporting both slides to PNG and checking diagram/text/legend all agree. Deck is now 18 slides (was 17); everything after the old slide 3 shifted down by one position. No other slide in the deck was touched — the earlier full-deck audit (title-wrap bugs, stale Slide 6 annotation, Slide 9 typo, ρ-range note on old Slide 4, missing titles on old Slides 15/16, run-on bullets) remains identified but explicitly deferred, not in scope for this session.

2. **Scoped a second, additive contribution: power-aware (reactive) AP association.** Re-examined `_associate_users()` in `env/cfj_env.py` against the 30-paper literature set and reconfirmed the exact mechanism: association is computed once in `reset()`, using a uniform-max-power assumption purely to break the association/power circular dependency (a fair, neutral first guess — not itself wrong), then locked for the rest of the episode while the RL agent picks its real (usually very different) power vector afterward. This is precisely the design Hoseini's own paper names as the cause of their scenario-5 secrecy drop: *"since the AP selection algorithm is done separately and first, by assuming uniform power allocation... the whole process is not optimal."*

   **The proposed fix:** don't change the RL agent or its action space at all — it still only outputs continuous power. Instead, move the association calculation out of `reset()` and into `step()`, and re-run the *same* formula using the agent's actual chosen powers instead of the fictional uniform-power guess, before scoring the reward. This is provably **weakly dominant** — for any given power vector, re-optimizing association against the real powers can never score worse than the old fixed guess, only equal or better, since the old guess is just one candidate inside the set the new step maximizes over. Directly testable against Hoseini's own documented failure (rerun a scenario-5-style "more APs" case, check whether the secrecy drop disappears).

   **Important scoping correction made this session:** this is *not* Hoseini's stated future work ("integrate the AP selection algorithm into the RL model") — that would mean the RL model itself learns/outputs the association decision, requiring a hybrid discrete+continuous action space (a real algorithm rebuild, previously scoped in the 2026-07-18 entry as a "soft continuous relaxation" future-work item, still open). What we're proposing keeps association as a fixed, non-learned formula — we only refresh *what powers get fed into it*. Framing this as "solving Hoseini's future work" would not survive scrutiny from anyone who reads their one-line future-work sentence; the correct claim is narrower: *"we resolve the specific association-power mismatch Hoseini documented, by making the existing association formula reactive to the agent's real output; full RL-learned AP selection, as they proposed, remains open future work requiring a hybrid action space."*

   **Status: scoped and agreed as Contribution 2, not yet implemented.** Per explicit sequencing from this session — (1) System Model slide text [done, this session], (2) literature-informed improvements to problem formulation/algorithm [this item, scoping done], (3) code review + retrain — no code has been changed yet for this. Next concrete step when resumed: move the `_associate_users()` call from `reset()` to `step()`, feed it real `powers` instead of `uniform_powers`, then retrain and compare against the current model.

3. **Tier-1 (paper-framing only, no code/retrain needed) improvements identified from the lit review, not yet written into `final_report.tex`:**
   - Name `R*` explicitly as an empirical, finite-sample (M=5) approximation of a small-τ CVaR objective (Wang, Kallus & Sun, 2023) rather than an unnamed heuristic.
   - Separate the two robustness mechanisms in the write-up: domain-randomization theory (Chen et al., 2022) justifies the *uniform sampling of σ per episode*; CVaR/worst-case theory (Wang-Kallus-Sun, Tanabe M2TD3, Lanier FARR) justifies the *min over M Eve samples given σ* — currently at risk of being conflated into one paragraph.
   - Justify the bounded `sigma_range=(0,10)` via Lanier et al.'s FARR (2022) — unbounded worst-case training produces needlessly conservative agents; the bound is a deliberate, literature-backed design choice, not an arbitrary hyperparameter.
   - Explicitly scope UA-SAC's robustness as empirical/Monte-Carlo, not certified, contrasting against Zhao et al. (2025, NQC-DDQN) and Miao et al. (2026), which offer certified/model-driven guarantees — pre-empt the obvious reviewer question rather than leave it for them to raise.
   - Resolve the Bouhafs/den Hartog (2020) Eve-detectability contradiction directly: their AP-selection paper assumes Eve's location is detectable, which conflicts with our premise; the reconciliation already drafted is that their approach requires trusting a separate wiretap-detection layer first, while our contribution covers the case where that layer isn't available or trusted.
   - Scope the contribution boundary against Tripathi et al. (2024): known-Eve-CSI is defensible for active/monitored/dual-role eavesdroppers (their explicit justification); our contribution targets the complementary case of a purely passive eavesdropper.

**Pending after this session:**
- Write the six Tier-1 items above into `final_report.tex` (no retrain needed for these)
- Implement and retrain the Contribution 2 (power-aware association) code change once given the go-ahead, then compare secrecy capacity against the current model, specifically under an added-APs scenario mirroring Hoseini's scenario 5
- Apply the deferred full-deck fixes (title-wrap bugs, Slide 6 stale annotation, Slide 9 typo, missing titles on Slides 15/16, run-on bullet text, stale plot data on the plot slides) once graphs are regenerated
- Everything else still open from the 2026-07-18 and 2026-07-22 entries above (orphan `liu` citation, honest ρ-range statement, Literature Review prose rewrite from the 30-paper notes)
...
### 2026-07-27 to 2026-07-28 — Contribution 2 Implemented, Retrain Saga, Full Report Rewrite, Repo Cleanup

**What was done, roughly in the order it happened:**

1. **Contribution 2 (power-aware association) implemented.** `_associate_users()` in `cfj_env.py` now takes the actual power vector as an argument and is called from `step()`/`evaluate_policy()` using the agent's real chosen powers, instead of being computed once in `reset()` under a fictional uniform-max-power guess. The weak-dominance property discussed in the 2026-07-26 entry holds by construction. Also added `_associate_sinr_only()` for a correct, separate "Normal Wi-Fi" baseline (Hoseini's actual SINR-only definition — the previous `eval_normal_wifi()` was quietly reusing the secrecy-aware association, and a *second* baseline, `eval_fixed_cfj()`, turned out to be a byte-for-byte duplicate of it, meaning one of the four comparison bars in every prior plot was never actually a distinct system). Implemented a real `eval_single_best_ap()` in its place.

2. **A real, previously-unnoticed CPU threading bug found and fixed.** `train.py` set `OMP_NUM_THREADS`/`MKL_NUM_THREADS`/`OPENBLAS_NUM_THREADS` to 8 (matching this machine's 8 *logical* processors, not its 4 *physical* cores), set them *after* numpy/torch were already imported (too late to take effect), and had a typo (`OPENBLAS_NTHREADS`, not a real variable). Worse: `SubprocVecEnv` workers inherited the same thread settings, so 4 worker processes each additionally tried to claim multiple BLAS threads — severe oversubscription on a 4-core machine. This had likely been silently slowing every training run in the project's history. Fixed: thread counts corrected to 4, each worker process explicitly capped to 1 thread via `threadpoolctl.threadpool_limits(1)`, and `env.close()` added after training (a second bug — leftover `SubprocVecEnv` worker processes from a finished training phase were never being shut down, still holding threads into the next phase). Verified with before/after process inspection: fps went from single digits back to 40+.

3. **Network architecture search.** Hoseini et al.'s reported "nine layers of depth" was first taken literally (9 raw weight layers, `[128]*9`) — this produced measurable training instability (critic loss failed to converge, rose over the second half of training instead of settling), consistent with documented findings that deep, unnormalized actor-critic networks develop unstable gradients as depth increases (Bjorck et al., "Towards Deeper Deep RL," verified via search, not previously in our 30-paper set — added to the report's bibliography for this one specific implementation-detail claim). Settled on a tapered 4-layer architecture, `[256,128,64,32]`, on the theory that MATLAB's layer-counting convention (counting activation layers separately from weight layers) makes "nine layers" plausibly correspond to ~4 real weight layers. This trained cleanly.

4. **Diagnosed why UA-SAC was losing to a trivial baseline.** Built a direct diagnostic: for a set of evaluation topologies, force every "idle" AP's power to zero (keeping the learned policy's serving-AP power unchanged) and compare secrecy against the policy's actual output. With the original shallow (2-layer) network, forced-zero beat the learned policy on 19/30 topologies; with the literal 9-layer network, 20/30 — meaning the agent was not reliably learning when jamming helps vs. hurts. This diagnostic became the standard check for every subsequent retrain.

5. **UA-SAC vs. Baseline SAC crisis, root-caused and fixed.** After the depth and threading fixes, a full retrain (100k timesteps each, matched architecture) showed Baseline SAC beating UA-SAC outright at every σ — the opposite of the paper's thesis. Root cause, confirmed rather than assumed: UA-SAC's training signal is substantially noisier than Baseline's (random σ every episode, a noisy observed Eve position, and a reward that is itself a stochastic minimum over M=5 samples — three stacked sources of variance on top of an already-hard continuous generalization problem), with the *same* 100k-timestep budget as the easy baseline. Fix: retrained UA-SAC with 300k timesteps (3x), and as a second, deliberately isolated change, set β=0 (disabling the ρ-scaled entropy layer, whose "avoid predictable jamming" justification doesn't hold against an eavesdropper who never adapts across episodes). This produced a clean, decisive result: UA-SAC beats Baseline SAC at every σ tested by a wide, statistically solid margin.

6. **Controlled ablation, run properly.** Because the fix in step 5 changed two variables at once (β and timesteps), a follow-up run trained UA-SAC with β=1 at the *same* 300k-timestep budget, isolating which variable actually mattered. Result: both β=0 and β=1 clearly beat Baseline SAC at 300k timesteps — confirming training budget was the dominant fix, not the entropy mechanism specifically. β=0 does still beat β=1 consistently, but the effect is much weaker than the UA-SAC-vs-baseline gap (win rate 51.4%/54.4% at σ=0/10m, vs. 61.8%/61.6% for UA-SAC vs. baseline) — reported honestly in the final paper as suggestive, not conclusive, given it rests on a single trained instance per configuration.

7. **Final verified numbers (all from the β=0, 300k-timestep UA-SAC model + the 100k-timestep Baseline SAC model, 1,000 shared evaluation topologies):** at σ=10m — Normal Wi-Fi 2.26, Single Best AP 3.68, Baseline SAC 3.98, UA-SAC 4.53 bps/Hz. UA-SAC beats Baseline SAC by 13.9% (paired t=10.04), and wins on 61.6% of individual topologies. Holds across the full σ∈[0,10]m sweep. Normalized-degradation metric (each system vs. its own σ=0 score) does *not* favor UA-SAC (97.5% vs. 97.9% retained) — reported as a real, disclosed limitation, not smoothed over: the advantage is in absolute capacity across the range, not in a shallower degradation slope.

8. **`docs/final_report.tex` fully rewritten.** Professional academic tone replacing the previous AI-humanized draft (preserved for reference, then deleted once superseded — see below); every equation individually checked against the live code, not just written to look plausible (caught and fixed a stale docstring in `uasac.py` that still described batch-averaged entropy scaling after the code had been changed to per-sample); every number recomputed from actual saved evaluation output (caught and fixed several rounding errors — e.g. "13.8%" should have been 13.9%, several Table 1 gain percentages were off by ~0.1-0.2 points); added the weak-dominance proposition and proof sketch for Contribution 2; added the full ablation study with significance statistics; corrected the ρ range table entry (was `[0,1]`, is actually `[0,0.2]`). Literature review expanded to genuinely cover all 30 papers from `literature_notes.md`, then found to be disproportionately long (2,249 words — larger than the Methodology and Results sections combined) relative to the paper's actual genre (an IEEE workshop-style conference paper, not a survey), and condensed to 1,050 words while keeping every citation's specific comparative point intact, not just its name.

9. **Repository cleanup.** Created `OLD/` and moved (via `git mv`, preserving history as renames) everything superseded: old compiled report PDFs, the abandoned IRS+DDPG proposal, 3 unused legacy Phase-1 models (`sac_noise_{2,5,10}.0` — `test.py` only ever loads `sac_noise_0.0`), the standalone `plot_convergence.py` (superseded by `train.py`'s own plot generation), a mistakenly-downloaded paper never part of the verified 30, and the entire pre-fix `results/` folder (every number in it predates the association correction and threading fix). Deleted `docs/humanized report.tex` (never git-tracked, fully superseded). Rewrote `README.md`, which had drifted badly out of date (stale numbers, wrong file paths, a different repo URL) — now matches current state, with verified, git-tracked image paths for the four current result plots.

**Target venue decision.** FIT 2026's deadline passed same-day as it was raised, and the two 2026-cycle flagship venues checked (IEEE Globecom 2026, deadline was May 3; IEEE CNS 2026, deadline extended to May 18) have also passed — moot regardless, since the paper was not in submittable shape at either date (stale numbers, broken baseline, undiagnosed association bug, no significance testing). **IEEE ICC 2027 is the live target: paper submission deadline October 2, 2026**, conference May 30–June 3, 2027, Washington DC — verified via web search, not assumed. Roughly 9 weeks of runway from 2026-07-28.

**Pending before an ICC 2027 submission would be genuinely ready:**
- Multi-seed replication (currently every number comes from a single trained instance per configuration — the single most likely reviewer objection)
- A scenario-5-style AP-density sweep (N=4,6,8...) to directly demonstrate Contribution 2 fixes the exact failure Hoseini documented, rather than relying solely on the weak-dominance proof
- Validate the Python port's baseline numbers against Hoseini's actual reported MATLAB results at matching N, as an independent reproduction check
- Confirm ICC 2027's specific track/CFP scope actually fits this paper before investing further
- Reformat to ICC's submission template and page limit
- Everything else still open from the 2026-07-26 entry (deferred pptx deck fixes, orphan citation, mobility/geometry-blind-ρ limitations) remains future work, not blocking for submission
